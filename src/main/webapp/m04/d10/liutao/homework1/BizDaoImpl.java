...
public class BizDaoImpl extends BaseDao implements BizDao {
    ...
    public int modification(NewsInfo ni) {
        ...
        try {
            ...
            pst = getCon().prepareStatement(sql);
            pst.setInt(1, ni.getType_id());
            pst.setString(2, ni.getNews_title());
            pst.setString(3, ni.getNews_author());
            pst.setString(4, ni.getNews_summary());
            pst.setString(5, ni.getNews_content());
            pst.setInt(6, ni.getNews_id());
            result = pst.executeUpdate();
        } catch (Exception e) {
            // TODO: handle exception
            e.printStackTrace();
    ...
    @Override
    public List<NewsInfo> getShowPageByCount(Page page) {
        List<NewsInfo> list = new ArrayList<NewsInfo>();
        try {
    ...
    public int getSumCount(Page page) {
        ...
        try {
            ...
            rs = pst.executeQuery();
            while (rs.next()) {
                page.setSumpage(rs.getInt(1));
                result = page.getSumpage();
            }
