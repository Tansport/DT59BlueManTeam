			...
			<span>商品名称：</span> <input name="queryProductName" type="text"
				...
				<c:if test="${listIpay != null }">
					...
					<c:forEach var="Ipay" items="${listIpay}">
						<c:if test="${Ipay eq '1' }">
							<option value="1" ${Ipay == 1 ? "selected=\"selected\"":"" }>未付款</option>
						</c:if>
						<c:if test="${Ipay eq '2' }">
							<option value="2" ${Ipay == 2 ? "selected=\"selected\"":"" }>已付款</option>
						</c:if>
					</c:forEach>
				</c:if>
		...
		<c:forEach var="bill" items="${list}" varStatus="status">
			<tr>
				...
				<td><span><a class="viewBill" href="javascript:;"
						...
						href="javascript:;" billid=${bill } billcc=${bill.billCode }><img
